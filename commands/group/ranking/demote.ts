...
const command: CommandFile = {
    run: async(interaction: Discord.CommandInteraction<Discord.CacheType>, client: BotClient, args: any): Promise<any> => {
        ...
        for(let i = 0; i < usernames.length; i++) {
            ...
            await client.logAction(`<@${interaction.user.id}> has demoted **${username}** from **${oldRoleName}** to **${potentialRole.name}** for the reason of **${reason}** in **${GroupHandler.getNameFromID(groupID)}**`);
        }
