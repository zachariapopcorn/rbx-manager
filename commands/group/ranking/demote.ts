...
export async function run(interaction: Discord.CommandInteraction, client: BotClient, args: any[]) {
    ...
    if(potentialRole.rank === 0) {
        let embed = client.embedMaker("Unable to Demote", "You're unable to demote this user because they are at the lowest role of the group", "error", interaction.user);
        return await interaction.editReply(embed);
    }
    ...
    if(client.isLockedRole(potentialRole)) {
        for(let i = currentRoleIndex - 1; i >= 0; i--) {
            ...
            if(potentialRole.rank === 0) {
                let embed = client.embedMaker("Unable to Demote", "You're unable to demote this user because all of the roles below them are locked", "error", interaction.user);
                return await interaction.editReply(embed);
            }
            if(!client.isLockedRole(potentialRole)) break;
        }
        let embed = client.embedMaker("Role Locked", `The role(s) below this user is locked, would you like to demote this user to **${potentialRole.name}**?`, "info", interaction.user);
        ...
        componentCollector.on('end', async(collectedButtons) => {
            ...
            if(button.customId === "yesButton") {
                try {
                    ...
                    let embed = client.embedMaker("Error", `There was an error while trying to demote this user`, "error", interaction.user);
            ...
            if(!didCancelOrError) {
                let embed = client.embedMaker("Success", `You have successfully demoted this user`, "success", interaction.user);
                await msg.edit(embed);
        ...
        try {
            ...
            let embed = client.embedMaker("Error", `There was an error while trying to demote this user: ${e}`, "error", interaction.user);
            return await interaction.editReply(embed);
        }
        let embed = client.embedMaker("Success", `You have successfully demoted this user`, "success", interaction.user);
