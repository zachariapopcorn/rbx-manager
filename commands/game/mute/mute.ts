    ...
    run: async(interaction: Discord.CommandInteraction<Discord.CacheType>, client: BotClient, args: any): Promise<any> => {
        ...
        for(let i = 0; i < usernames.length; i++) {
            ...
            try {
                ...
                await database.setModerationData(universeID, robloxID, {banData: {isBanned: oldData.banData.isBanned, reason: oldData.banData.reason}, muteData: {isMuted: true, reason: reason}});
            } catch(e) {
                logs.push({
                    username: username,
                    status: "Error",
